...
Internally, `MODE=elp` and `MODE=cpu` define `-DUSE_ELP_TIMER` and `-DUSE_CPU_TIMER` (C/C++) or select the corresponding Fortran timer, and `MODE=gprof` adds the `-pg` compiler flag. `bash run.sh` also accepts the same `MODE=...` argument; only `MODE=gprof` changes its behavior (it runs `gprof` after execution).
...
4. Compare the elapsed times and consider which routine is more expensive and why.
