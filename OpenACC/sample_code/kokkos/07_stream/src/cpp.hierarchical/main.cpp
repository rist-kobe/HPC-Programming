...
int main (int argc, char **argv)
{
  ...
  if ( vl <= 0 ) {
     ...
     // nlg = ( nsize % vl == 0 ) ? nsize/vl : nsize/vl + 1; //TODO: To check the result when nsize % vl != 0 
     if ( nsize % vl != 0 ) {
  ...
  //--------------------------------------------------------------------
  if ( vl > 0 ) { // leage size and vector length are explicitly set.
  //--------------------------------------------------------------------
  fprintf(stdout," Leage size: %d, Team size: %d, Vector length: %d\n", nlg, 1, vl);
  ...
  for (int k = 0; k < NTIMES; ++k) {
     ...
     Kokkos::parallel_for("Copy2",
        Kokkos::TeamPolicy<ExecSpace>(nlg, 1, vl), 
	KOKKOS_LAMBDA (const typename Kokkos::TeamPolicy<ExecSpace>::member_type &team) {
	   const int lrank = team.league_rank();
           const int ista  = lrank * vl;
	   const int iend  = ( ista + vl > nsize ) ? nsize : ista+vl;
	   // [=] (copy) may also work
	   Kokkos::parallel_for(Kokkos::TeamVectorRange(team,ista,iend), [&] (const int &i) {
              c(i) = a(i); 
	   });
     });
     Kokkos::fence();
...
#endif
  //--------------------------------------------------------------------
  } else { // Kokkos will automatically control everything.
  //--------------------------------------------------------------------
