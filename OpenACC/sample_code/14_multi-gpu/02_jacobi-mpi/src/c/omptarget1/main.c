...
#endif
    nrmsq_loc = 0.0;
    #pragma omp target data map(to:phie[0:nx_loc*ny_loc],rho[0:nx_loc*ny_loc]) \
    map(from:phio[0:nx_loc*ny_loc])
    #pragma omp target
...
#endif
    #pragma omp target data map(to:phio[0:nx_loc*ny_loc]) map(from:phie[0:nx_loc*ny_loc])
    #pragma omp target 
