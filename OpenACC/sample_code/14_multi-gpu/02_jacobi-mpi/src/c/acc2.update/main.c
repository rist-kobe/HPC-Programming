...
int main (int argc, char **argv)
{
...
#endif
  #pragma acc data copyin(rho[0:nx_loc*ny_loc]) copy(phie[0:nx_loc*ny_loc]) \
  create(send_buff[0:max_buffer_size],recv_buff[0:max_buffer_size],phio[0:nx_loc*ny_loc])
  {
...
#endif
    nrmsq_loc = 0.0;
    #pragma acc data copy(nrmsq_loc) \
    present(phio[0:nx_loc*ny_loc],phie[0:nx_loc*ny_loc],rho[0:nx_loc*ny_loc])
    #pragma acc kernels 
    #pragma acc loop independent collapse(2) reduction(max:nrmsq_loc)
    for (int ix = 1; ix < nx_loc-1; ++ix ) {
      for (int iy = 1; iy < ny_loc-1; ++iy ) {
  ...
  if( coords[1] == 0 || coords[1] == npy-1) {
    #pragma acc data present(phio[0:nx_loc*ny_loc]) 
    #pragma acc kernels
...
#endif
    #pragma acc data present(phie[0:nx_loc*ny_loc],rho[0:nx_loc*ny_loc])
    #pragma acc kernels
    #pragma acc loop independent collapse(2)
    for (int ix = 1; ix < nx_loc-1; ++ix ) {
      for (int iy = 1; iy < ny_loc-1; ++iy ) {
  ...
  nvtxRangeEnd(id3);
    ...
    /* packing */
    #pragma acc data present(phie[0:nx_loc*ny_loc],send_buff[0:max_buffer_size])
    #pragma acc kernels
    #pragma acc loop independent 
    for (int iy = 1; iy < ny_loc-1; ++iy) {
    ...
    #pragma acc loop independent 
    for (int iy = 1; iy < ny_loc-1; ++iy) {
       phie[IDX(nx_loc-1,iy,ny_loc)] = recv_buff[iy];
    }
    ...
    #pragma acc loop independent 
    for (int iy = 1; iy < ny_loc-1; ++iy) {
       phie[IDX(0,iy,ny_loc)] = recv_buff[iy];
    }
    ...
    #pragma acc loop independent 
    for (int ix = 1; ix < nx_loc-1; ++ix) {
       phie[IDX(ix,ny_loc-1,ny_loc)] = recv_buff[ix];
    }
    ...
    #pragma acc loop independent 
    for (int ix = 1; ix < nx_loc-1; ++ix) {
       phie[IDX(ix,0,ny_loc)] = recv_buff[ix];
    }
