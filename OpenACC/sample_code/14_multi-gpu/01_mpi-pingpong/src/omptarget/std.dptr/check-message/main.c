...
/* Reference
   * Tom Deakin and Timothy G. Mattson, "Programming your GPU with OpenMP: Performance Portability fot GPUs"
     (MIT, 2023) Chap.9. 
*/
#include <mpi.h>
#include <stdio.h>
...
int main (int argc, char **argv)
{
   ...
   // a single ping-pong
   {
      ...
      if ( me == 0 ) {
         ...
         #pragma omp target data map(tofrom:src_buf[0:size],rcv_buf[0:size]) device(igpu) 
         {
	    #pragma omp target data use_device_ptr(src_buf)
	    {
            MPI_Send (src_buf, size, MPI_INT, dest, 100, MPI_COMM_WORLD);
            } 
        
	    #pragma omp target data use_device_ptr(rcv_buf)
	    {
