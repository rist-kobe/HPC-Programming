! Copyright 2024 Research Organization for Information Science and Technology
module mymod
  implicit none
  ! ATTENTION: The work array u is a module variable. In an OpenMP
  ! parallel region it is SHARED among the threads, and concurrent
  ! calls to func race on it.
  integer,private :: u(2)

contains
  ! Splits the range [1, ns] into contiguous blocks, one per thread.
  ! The block of thread tid is the inclusive range [ifrom, ito].
  subroutine decomp_size (ns, tid, nt, ifrom, ito)
     integer,intent(in) :: ns, tid, nt
     integer,intent(out) :: ifrom, ito
     integer :: delta
     delta = 1 + ns / nt
     ifrom = 1 + tid * delta
     ito   = MIN(ifrom+delta-1, ns)
  end subroutine
 
  ! Indeed, the working array, u, is **NOT** needed. 
  ! This implementation is nothing more than an example.
  ! Because u is a module variable, it is SHARED among the threads; this
  ! data race is why the caller must protect func by a critical construct.
  function func (a, ifrom, ito) 
    integer :: func
    integer,contiguous,intent(in) :: a(:)
    integer,intent(in) :: ifrom
    integer,intent(in) :: ito

    integer :: i, ifrom1, ifrom2

    if ( mod(ifrom,2) .eq. 1 ) then
       ifrom1 = ifrom
       ifrom2 = ifrom + 1
    else
       ifrom1 = ifrom + 1
       ifrom2 = ifrom
    end if

    u(1) = 0;
    do i = ifrom1, ito, 2
       u(1) = u(1) + a(i)
    end do

    u(2) = 0;
    do i = ifrom2, ito, 2
       u(2) = u(2) - a(i)
    end do

    func = u(1) + u(2)
  end function func

  ! A better implementation: the work array, u, is a local variable, so
  ! this function has no shared state and is safe to call concurrently.
  function func_local (a, ifrom, ito)
    integer :: func_local
    integer,contiguous,intent(in) :: a(:)
    integer,intent(in) :: ifrom
    integer,intent(in) :: ito

    integer :: i, ifrom1, ifrom2
    integer :: u(2)

    if ( mod(ifrom,2) .eq. 1 ) then
       ifrom1 = ifrom
       ifrom2 = ifrom + 1
    else
       ifrom1 = ifrom + 1
       ifrom2 = ifrom
    end if

    u(1) = 0;
    do i = ifrom1, ito, 2
       u(1) = u(1) + a(i)
    end do

    u(2) = 0;
    do i = ifrom2, ito, 2
       u(2) = u(2) - a(i)
    end do

    func_local = u(1) + u(2)
  end function func_local
end module mymod
