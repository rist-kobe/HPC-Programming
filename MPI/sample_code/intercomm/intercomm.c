...
int main(int argc,char *argv[]){
  ...
  if(mpi_comm_group1!=MPI_COMM_NULL){
    ...
    MPI_Intercomm_create(mpi_comm_group1,0,MPI_COMM_WORLD,ranks2[0],tag,&mpi_comm_inter);
    MPI_Gather(sendbuf,nd,MPI_INT,recvbuf,nd,MPI_INT,rootS,mpi_comm_inter);
  }
  if(mpi_comm_group2!=MPI_COMM_NULL){
    ...
    fprintf(fp0,"\n");
    fprintf(fp0," rankS=%2d sizeS=%2d\n",rankS,sizeS);
    MPI_Intercomm_create(mpi_comm_group2,0,MPI_COMM_WORLD,ranks1[0],tag,&mpi_comm_inter);
    if(rankS==rootS){
      MPI_Gather(sendbuf,nd,MPI_INT,recvbuf,nd,MPI_INT,MPI_ROOT,mpi_comm_inter);
    }else{
      MPI_Gather(sendbuf,nd,MPI_INT,recvbuf,nd,MPI_INT,MPI_PROC_NULL,mpi_comm_inter);
    }
  }
  fprintf(fp0,"\n");
  for(i=0;i<sizeW;i++){
